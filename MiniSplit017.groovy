 ...
 *
 *  Jul 09, 2022	v0.1.7	Add support for Dew Point conntrol (experimental) 
...
//			Dew Point Options added Jul 09, 2022, copied from development Dew Point app
...
//			globalThermostat.setThermostatFanMode('Dry')
	...
	def calcDew(adjustDev=true,Tx=tempSensor.currentValue("temperature"), RH= humidSensor.currentValue("humidity")) 
		{
	...
	//	log.debug "${dewPoint} ${averageDev.currentValue('temperature')}"
		if (adjustDev)
			{
		 	def averageDev = getChildDevice("DEWPoint_${app.id}")
			if (averageDev && dewPoint != averageDev.currentValue('temperature'))
				{
	 			if (settings.logDebugs) log.debug "update dewPoint: ${dewPoint} from ${averageDev.currentValue('temperature')} Humid: ${RH} Temp: ${tempSensor.currentValue('temperature')}"
